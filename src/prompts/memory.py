...
def build_memory_prompt(category_tags: list[str] | None = None) -> str:
    ...
    prompt = (
        ...
        "### TODOs and reminders\n"
        "- Use `create_todo` for tasks and `create_reminder` for deadline-based reminders.\n"
        "- Set `owner` to `user` (default) for things the user needs to do, or `agent` for your own tasks.\n"
        "- **Todos are lists**: store items as markdown checkboxes in the content field, "
        "e.g. `- [ ] Buy milk\\n- [ ] Call dentist`. A single-item todo is fine too.\n"
        "- To complete one item from a list: `complete_task(memory_id, item='...')` — "
        "removes the matched item; deletes the todo when the last item is done.\n"
        "- To complete an entire task at once: `complete_task(memory_id)` with no `item`.\n"
        ...
        "- Do NOT list all tasks on startup. Only proactively mention reminders that are **overdue** "
